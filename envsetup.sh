...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^intense_") ; then
       INTENSE_BUILD=$(echo -n $1 | sed -e 's/^intense_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $INTENSE_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       INTENSE_BUILD=
    fi
    export INTENSE_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${INTENSE_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    INTENSE_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/intense/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the INTENSE model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch intense_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the INTENSE github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var INTENSE_VERSION)
    ...
    if (adb shell getprop ro.intense.device | grep -q "$INTENSE_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $INTENSE_BUILD, run away!"
    fi
}
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                intense|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $INTENSE_FIXUP_COMMON_OUT ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.intense.device | grep -q "$INTENSE_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/intense/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
